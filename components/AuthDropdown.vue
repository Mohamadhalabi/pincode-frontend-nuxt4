<template>
  <div v-if="loggedIn" class="relative" @mouseleave="open=false">
    <button class="pill bg-gray-100" @mouseenter="open=true">
      ...
      <span>{{ me?.name || $t('nav.account') }}</span>
    </button>

    <div v-show="open" class="absolute right-0 mt-2 w-56 card p-2 z-50">
      <div class="px-3 py-2 text-sm text-gray-600">
        {{ $t('auth.signedInAs') }} <b>{{ me?.email }}</b>
      </div>
      
      <div class="h-px bg-gray-200 my-2"></div>
      
      <NuxtLinkLocale to="/lookups/new" class="block px-3 py-2 rounded-lg hover:bg-gray-50">
        {{ $t('btn.newLookup') }}
      </NuxtLinkLocale>
      
      <button class="block w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-red-600" @click="logout">
        {{ $t('auth.logout') }}
      </button>
    </div>
  </div>

  <NuxtLinkLocale v-else class="btn" to="/login">
    {{ $t('auth.login') }}
  </NuxtLinkLocale>
</template>
