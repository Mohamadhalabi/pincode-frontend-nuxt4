...
// ✅ reactive login state from composable (this fixes Hello after logout)
...
onMounted(async () => {
  try {
    const token =
      process.client
        ? (localStorage.getItem('pk:token') || localStorage.getItem('auth_token'))
        : null
...
const currentLocale = computed(
  () =>
    safeLocales.value.find(l => l.code === localeCode.value) ??
    { code: localeCode.value, dir: 'ltr' }
)
    ...
    <Transition name="slide-fade">
      <div v-if="mobileOpen" class="md:hidden border-t border-gray-100 bg-white shadow-xl absolute w-full left-0">
        <div class="mx-auto px-4 py-4 flex flex-col text-sm text-gray-700 space-y-1">
          ...
          <!-- Buy Old Tokens -->
          <button
            class="group w-full py-3 px-3 rounded-xl text-left hover:bg-[#f0fdfc] flex items-center gap-3 transition-colors"
            @click="mobileOpen=false; go('/product/kia-hyundai-pincode-old')"
          >
            <div class="p-2 bg-gray-50 rounded-lg group-hover:bg-white group-hover:text-[#0e5e6f] transition-colors">
          ...
          <button
            class="group w-full py-3 px-3 rounded-xl text-left hover:bg-[#f0fdfc] flex items-center gap-3 transition-colors"
            @click="mobileOpen=false; go('/product/kia-hyundai-pincode-new')"
          >
            <div class="p-2 bg-gray-50 rounded-lg group-hover:bg-white group-hover:text-[#0e5e6f] transition-colors">
              <svg class="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                stroke-linecap="round" stroke-linejoin="round">
          ...
          <button
            class="group w-full py-3 px-3 rounded-xl text-left hover:bg-[#f0fdfc] flex items-center gap-3 transition-colors"
            @click="mobileOpen=false; go('/services/kia-hyundai-before-2017')"
          >
            <div class="p-2 bg-gray-50 rounded-lg group-hover:bg-white group-hover:text-[#0e5e6f] transition-colors">
          ...
          <button
            class="group w-full py-3 px-3 rounded-xl text-left hover:bg-[#f0fdfc] flex items-center gap-3 transition-colors"
            @click="mobileOpen=false; go('/services/kia-hyundai-after-2017')"
          >
            <div class="p-2 bg-gray-50 rounded-lg group-hover:bg-white group-hover:text-[#0e5e6f] transition-colors">
              <svg class="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"
                stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="pt-2">
            <button
              v-if="isLoggedIn"
              class="btn w-full"
              @click="mobileOpen=false; go('/account')"
            >
              {{ helloText }}
            </button>
            <button
              v-else
              class="btn w-full"
              @click="mobileOpen=false; go('/login')"
            >
              {{ t('auth.login') || 'Login' }}
            </button>
          </div>
