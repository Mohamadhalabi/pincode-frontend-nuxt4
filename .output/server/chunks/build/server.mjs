import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { getCurrentInstance, hasInjectionContext, inject, toRef, isRef, shallowRef, computed, defineComponent, createElementBlock, provide, cloneVNode, h, defineAsyncComponent, unref, shallowReactive, ref, Suspense, Fragment, useSSRContext, createApp, createVNode, Text, mergeProps, withCtx, onErrorCaptured, onServerPrefetch, resolveDynamicComponent, reactive, effectScope, isReadonly, isShallow, isReactive, toRaw, nextTick, getCurrentScope } from 'vue';
import { f as createError$1, s as sanitizeStatusCode, k as getContext, $ as $fetch, l as baseURL, m as createHooks, n as executeAsync, o as getRequestProtocol, t as toRouteMatcher, q as createRouter$1, r as defu, h as destr, v as klona, w as getRequestHeaders, x as getRequestHeader, y as isEqual$1, z as setCookie, A as getCookie, B as deleteCookie } from '../nitro/nitro.mjs';
...
const api_jy9dy79pM_nYQuRKPrcfNg56p_gKkzw9SekUTZgAenc = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
  ...
  const baseURL2 = config.public?.apiBase || config?.apiBase || "/api";
  const api = async (path, opts = {}) => {
    const url = path.startsWith("http") ? path : `${baseURL2.replace(/\/$/, "")}/${path.replace(/^\//, "")}`;
    const headers = {
    ...
    const fetchOpts = {
      ...
      headers,
      credentials: "omit",
      // 👈 important for your JWT / CORS setup
      mode: "cors"
    };
    if (opts.body !== void 0) {
      fetchOpts.body = headers["Content-Type"] === "application/json" && !(opts.body instanceof FormData) ? JSON.stringify(opts.body) : opts.body;
    }
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-white text-gray-900" }, _attrs))}>`);
      _push(ssrRenderComponent(_component_NuxtLoadingIndicator, { height: 8 }, null, _parent));
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-CuFA4TG_.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-BwTb5CBo.mjs'));
